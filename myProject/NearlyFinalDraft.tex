...
\section{Zero Knowledge Compilers}
	...
	\begin{center}
		$\text{ZKP}[(w_{1},w_{2}):x_{1} = \phi_{1}(w_{1}) \land  x_{2} = \phi_{2}(w_{2}) \land w_{1} = aw_{2}]$ 
	\end{center}
	
	What this means is \textit{"proof of knowledge of $w_{1}, w_{2}$ such that 
	$x_{1} = \phi_{1}(w_{1}), x_{2} = \phi_{2}(w_{2})$ and $w_{1} = aw_{2}$}.
	...
	% Protocols are. I also want to talk about the compiler that the authors made
		...
		\end{equation*}
		In the second equation, $HLR(w_{1},...,w_{m})$ denotes a system of 
