...
\section{Zero Knowledge Protocols}
	...
	A \textit{cycle} is a sequence of vertices which starts and ends at
	the same vertex, and two consecutive vertices in the sequence are
	adjacent (connected) to each other in the graph. A \textit{Hamiltonian
	path}, is a sequence of vertices in which each vertex in the graph is
	listed exactly once, and similarly to above two consecutive vertices in
	the sequence are adjacent in the graph. Finally, a \textit{Hamiltonian cycle}
	...
	%% Find a Picture of a Hamiltonian Cycle to put in here.
	...
	\begin{itemize}
		...
		$H$, while at the same time $V$ has no knowledge of $H$.
...
\section{Compilers}
	Fundamentally, what a compiler does is take a language and convert it into
	another language. For example, the java compiler takes the java programming
	language as input and outputs java bytecode. There are many different types of
    ...
    specification,  written in languages designed specifically for this problem,
    and output an implementation of the given specification in a high-level language,
    usually C++ or Java.
